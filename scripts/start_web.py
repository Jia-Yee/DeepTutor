...
def main() -> None:
    ...
    # Determine API base URL for frontend
    # Priority: NEXT_PUBLIC_API_BASE env var > NEXT_PUBLIC_API_BASE_EXTERNAL > localhost
    api_base = os.environ.get("NEXT_PUBLIC_API_BASE") or os.environ.get("NEXT_PUBLIC_API_BASE_EXTERNAL")
    
    if not api_base:
        # Default to localhost for local development
        api_base = f"http://localhost:{backend_port}"
        log_info(f"Using default API base: {api_base}")
        log_info(f"To enable remote access, set NEXT_PUBLIC_API_BASE in your .env file")
    else:
        log_success(f"Using configured API base: {api_base}")
    ...
    env_local_path.write_text(
        ...
        f"# To configure for remote access, set NEXT_PUBLIC_API_BASE in your .env file\n"
